...
class CobottaGripperAction(object):
  ...
  def start(self):
    ...
    while not rospy.is_shutdown():
      ...
      # self._rate.sleep()
      # self.place(192.06,51.07,10,-179.45,0.25,179.54, 50)
      # self._rate.sleep()
      r.sleep()
    self.motion_down()
    exit(-1)
  ...
  def grasp_cb(self, msg):
    ...
    T_camera_matrix = quaternion_matrix([T_camera.transform.rotation.w, 
                                         T_camera.transform.rotation.x,
    ...
    for grasp in msg.grasps:
      ...
      rospy.loginfo("hand position: {}".format(T_hand_in_base[:3,3]))
